...
export async function getStaticProps({ params }) {
 dbConnect();
  ...
  const fetchrecentposts = Post.find({ '_id': { $ne: id }, hidden: "false" }).sort({ date: -1 }).limit(10);
  const fetchrelatedposts = Post.find({ '_id': { $ne: id }, hidden: "false", tags: post.tags[0] }).limit(10)
  const result= await Promise.all([fetchauthor,fetchauthortags,fetchrecentposts,fetchrelatedposts]).then(([fauthor,fauthortags,frecentposts,frelatedposts])=>{
